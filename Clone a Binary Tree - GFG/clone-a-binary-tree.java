...
class Solution{
    public Tree cloneTree(Tree tree){
       ...
       preorder(tree,map);
       return buildTree(tree,map);
       
     }
     Tree buildTree(Tree root, Map<Tree, Tree> map){
         if(root==null)
         return null;
         Tree clone = map.get(root);
         clone.left = buildTree(root.left, map);
         clone.right = buildTree(root.right, map);
         if(root.random!=null)
    ...
    void preorder(Tree root, Map<Tree, Tree> map){
        if(root==null)
        return;
        Tree clone = new Tree(root.data);
        map.put(root, clone);
